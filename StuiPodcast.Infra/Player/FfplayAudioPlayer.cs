...
namespace StuiPodcast.Infra.Player
{
    ...
    // no live pause seek speed or volume; changes apply on restart
    // safe argument passing via argumentlist and event dispatch outside locks
    public sealed class FfplayAudioPlayer : IAudioPlayer
    {
        ...
        public void SeekTo(TimeSpan position)
        {
            ...
            lock (_gate)
            {
                ...
                _proc = started;
                State.IsPlaying = true;
                // ui estimate only; ffplay does not report position
        ...
        public void SetVolume(int vol0to100)
        {
            ...
            if (raise) FireStateChanged(); // takes effect on next start
        }
